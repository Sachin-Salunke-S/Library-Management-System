...
btn_hlb_bg = 'Grey'  # Background color for Head Labels and Buttons
...
edit_menu.add_command(label = "Export", command=export_to_csv)
edit_menu.add_command(label = "Import", command = import_csv_to_db)
edit_menu.add_command(label = 'Delete Inventory', command = delete_inventory)
edit_menu.add_command(label = "Show", command = display_records)
...
Button(RT_frame, text='Search', font=btn_font, bg=btn_hlb_bg, width=17, command=search).place(x=178, y=30)
